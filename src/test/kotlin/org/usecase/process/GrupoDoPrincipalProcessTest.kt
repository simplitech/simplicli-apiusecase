 ...
 */
class GrupoDoPrincipalProcessTest : AppTest() {
    private val subject = GrupoDoPrincipalProcess(context)

    private fun createModel(id: Long = 0) = GrupoDoPrincipal().apply {
        idGrupoDoPrincipalPk = id
        titulo = "1"
    }
    ...
    @Test
    fun testList() {
        GrupoDoPrincipalListParam().also {
    ...
    @Test
    fun testValidateGrupoDoPrincipal() {
        assertFailsWith<BadRequestException> {
            val model = createModel().apply { titulo = "" }
            subject.validateGrupoDoPrincipal(model, updating = true)
        }
